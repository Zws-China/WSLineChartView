...
CGFloat SVProgressHUDRingRadius = 3;
...
@property (nonatomic, strong, readonly) TBActivityView *spinnerView;
...
+ (void)showErrorWithStatus:(NSString *)string {
    ...
    [[self sharedView] showStringWithStatus:string duration:displayInterval];
}
...
-(void)showStringWithStatus:(NSString *)string duration:(NSTimeInterval )duration{
    self.progress = -1;
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.1f + 0.6, 5.0);
}
...
//        ((UIView *)hudView).translucent = YES;
        ((UIView *)hudView).backgroundColor= self.hudBackgroundColor;
#else
        hudView = [[UIView alloc] initWithFrame:CGRectZero];
...
- (TBActivityView *)spinnerView {
    if (spinnerView == nil) {
        spinnerView = [[TBActivityView alloc] initWithFrame:CGRectMake(0, 0, 320, 20)];
